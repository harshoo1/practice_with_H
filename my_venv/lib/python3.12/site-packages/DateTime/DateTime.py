...
def _calcIndependentSecondEtc(tz, x, ms):
    ...
    micros = (
        (x + 86400 - fset) * 1000000
        + long(round(ms * 1000000.0))
        - long(EPOCH * 1000000.0)
    )
    s = d - math.floor(d)
    return (s, d, t, micros)
...
class DateTime:
    ...
    def __getstate__(self):
        return (self._micros, getattr(self, "_timezone_naive", False), self._tz)
    ...
    def __setstate__(self, value):
        if isinstance(value, tuple):
            ...
            self._parse_args(micros / 1000000.0, tz)
            self._micros = micros
    ...
    def _parse_args(self, *args, **kw):
        ...
        if ac == 10:
            ...
            elif isinstance(arg, basestring) and arg.lower() in _TZINFO._zidx:
    ...
    def toZone(self, z):
        ...
        try:
            ...
            return self.__class__(
                yr, mo, dy, hr, mn, sc, tz, t, self._d, self.time, micros, tznaive
            )
        except Exception:
            ...
            sc = self._second
            ms = sc - math.floor(sc)
            x = _calcDependentSecond2(
                self._year, self._month, self._day, self._hour, self._minute, sc
            )
            x_new = x + tzdiff
    ...
    def isCurrentHour(self):
        ...
        return (
            gmt[0] == self._year
            and gmt[1] == self._month
            and gmt[2] == self._day
            and gmt[3] == self._hour
        )
    ...
    def utcdatetime(self):
        ...
        dt = datetime(
            utc._year, utc._month, utc._day, utc._hour, utc._minute, second, microsec
        )
        return dt
    ...
    def __add__(self, other):
        ...
        yr, mo, dy, hr, mn, sc = _calcYMDHMS(x, ms)
        return self.__class__(
            yr, mo, dy, hr, mn, sc, self._tz, t, d, s, tmicros, self.timezoneNaive()
        )
    ...
    def __str__(self):
        ...
        if s == int(s):
            ...
            return "%4.4d/%2.2d/%2.2d %2.2d:%2.2d:%06.6f %s" % (y, m, d, h, mn, s, t)
    ...
    def __hash__(self):
        ...
        return int(
            ((self._year % 100 * 12 + self._month) * 31 + self._day + self.time) * 100
        )
