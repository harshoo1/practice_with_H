...
class DateTimeTests(unittest.TestCase):
    ...
    def testDSTInEffect(self):
        ...
        self.assertEqual(dt.toZone("GMT").hour(), 19, (dt, dt.toZone("GMT")))
    ...
    def testDSTNotInEffect(self):
        ...
        self.assertEqual(dt.toZone("GMT").hour(), 20, (dt, dt.toZone("GMT")))

    def testAddPrecision(self):
        # Precision of serial additions
        dt = DateTime()
        self.assertEqual(str(dt + 0.10 + 3.14 + 6.76 - 10), str(dt), dt)
    ...
    def testConsistentSecondMicroRounding(self):
        ...
        self.assertEqual(int(dt.second() * 1000000), dt.micros() % 60000000)
        ...
        for year in (1990, 2001, 2020):
            for month in (1, 12):
                for day in (1, 12, 28, 31):
                    ...
                    d_int = DateTime(
                        "%d.%d.%d" % (day, month, year), datefmt="international"
                    )
    ...
    def test_calcTimezoneName(self):
        ...
        try:
            ...
            self.fail(
                "Zope Collector issue #484 (negative time bug): " "TimeError raised"
            )
    ...
    def testTimezoneNaiveHandling(self):
        ...
        self.assertFalse(
            dt.timezoneNaive(), "error with naivity handling in __parse_iso8601"
        )
        ...
        self.assertFalse(dt.timezoneNaive(), "error with naivity handling in _parse")
        dt = DateTime("2007/10/04 15:12:33.487618")
        self.assertTrue(dt.timezoneNaive(), "error with naivity handling in _parse")
        dt = DateTime()
        self.assertFalse(dt.timezoneNaive(), "error with naivity for current time")
